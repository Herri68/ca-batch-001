...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "CustomApp — Learn at your own pace" },
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const featured = await Promise.all(courses.slice(0, 3).map(async (course) => ({
    ...course,
...
export default function Home({ loaderData }: Route.ComponentProps) {
  ...
  function toggleDarkMode() {
    ...
    try {
      localStorage.setItem("customapp-theme", next ? "dark" : "light");
    } catch {}
  }
