...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const coursesWithProgress = await Promise.all(enrolledCourses.map(async (enrollment) => {
    const [progress, completedLessons, totalLessons, nextLesson] = await Promise.all([
      calculateProgress(currentUserId, enrollment.courseId, false, false),
      getCompletedLessonCount(currentUserId, enrollment.courseId),
      getTotalLessonCount(enrollment.courseId),
      getNextIncompleteLesson(currentUserId, enrollment.courseId),
    ]);
