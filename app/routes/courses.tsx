...
export function meta() {
  return [
    { title: "Browse Courses — CustomApp" },
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const [courses, currentUserId, country, categories] = await Promise.all([
    buildCourseQuery(search, category, CourseStatus.Published, "newest", 50, 0),
  ...
  if (currentUserId) {
    ...
    const progressEntries = await Promise.all(enrollments.map(async (enrollment) => {
      const [progress, completedLessons] = await Promise.all([
        calculateProgress(currentUserId, enrollment.courseId, false, false),
        getCompletedLessonCount(currentUserId, enrollment.courseId),
      ]);
      return [enrollment.courseId, { progress, completedLessons }] as const;
    }));
    for (const [k, v] of progressEntries) progressMap.set(k, v);
  }
  ...
  const coursesWithLessonCount = await Promise.all(courses.map(async (course) => {
    ...
    const pppPrice = course.pppEnabled
      ...
      : course.price;
    const rating = ratingsMap.get(course.id) ?? { average: null, count: 0 };
    return {
      ...course,
