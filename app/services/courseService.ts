...
export function getCourseById(id: number) {
  return db.select().from(courses).where(eq(courses.id, id)).limit(1).then(r => r[0]);
}

export function getCourseBySlug(slug: string) {
  return db.select().from(courses).where(eq(courses.slug, slug)).limit(1).then(r => r[0]);
}
...
export function getCoursesByStatus(status: CourseStatus) {
  return db.select().from(courses).where(eq(courses.status, status));
}
...
export async function buildCourseQuery(
  ...
  return sorted.limit(limit).offset(offset);
}
